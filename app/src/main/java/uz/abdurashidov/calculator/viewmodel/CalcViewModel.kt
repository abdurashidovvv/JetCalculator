package uz.abdurashidov.calculator.viewmodel
...
class CalcViewModel : ViewModel() {
    ...
    internal val viewState = state
        .map { state ->
            ...
            val operator = state.operator?.smybol.orEmpty()
            val num2 = state.num2
    ...
    fun dispatch(action: ActionType) {
        when (action) {
            ActionType.Calculate -> onCalculateClicked()
            ActionType.Clear -> onClearClicked()
            ActionType.Decimal -> onDecimalClicked()
            ActionType.Delete -> onDeleteClicked()
